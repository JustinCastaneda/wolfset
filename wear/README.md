...
- `ui/SetScreen.kt`, `ui/TimerScreen.kt`, `ui/DoneScreen.kt`, `ui/IdleScreen.kt` — one per
  thing the session is doing; `WatchApp.kt` picks. `ui/SetPager.kt` puts the Actions panel
  (`ui/ActionsScreen.kt`: Skip Set / End Workout) a swipe left of the set;
...
A set (`"screen":"set"`, add `"setNo":3,"dayDone":1,"dayTotal":10` to see a skipped pip and
the End Workout copy; swipe left for the panel), or the summary:
